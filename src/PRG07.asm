...
ExecGameLoopback:
	...
	INY
	_LDAY	$00 		;adjust area object offset based on
...
FindLoop		
	LDA	#03 ;bLoopDataBank
	JSR	UnsafeSwitchLoBank
	LDA	LoopCommandLoPtr
	STA	$00
	LDA	LoopCommandHiPtr
	STA	$01
	LDY	#00
	_LDAY	$00
	CMP	CurrentPageLoc
...
CheckPageCtrlRow:
        ...
        jmp _ProcLoopCommand      ;jump back to process loop commands again
...
EDITOR.NES_X_SPINY_EGG_PATCH_PLACE.FFFF:
...
EDITOR.NES_X_BOWSER_HAMMERS.FFFF:
           cmp #World6
...
EDITOR.NES_X_BOWSER_IDENTITIES.FFFF:
...
EDITOR.NES_X_WARP_ZONE_WORLDS.FFFF:
...
CheckBowserFront:
             ...
             sta $04
EDITOR.NES_X_GFX_ENEMY_OFFSETS.FFFF:
...
EDITOR.NES_X_PLAYER_SPRITE_TABLE.FFFF:
...
DrawPlayerLoop:
EDITOR.NES_X_PLAYER_SPRITE_TILES.FFFF:
...
lbLoopFindPage:
	...
	CLC
	ADC	$00
	STA	LoopCommandLoPtr
	LDA	#00
	ADC	$01
	STA	LoopCommandHiPtr
	INY
	LDA	#01
	BNE	lbExit1
...
PwrUpJmp:
           ...
           bcs _PutBehind             ;if star or 1-up, branch ahead
           lda PlayerStatus          ;otherwise check player's current status
           cmp #$02
           bcc _StrType               ;if player not fiery, use status as power-up type
           lsr                       ;otherwise shift right to force fire flower type
_StrType:   sta PowerUpType           ;store type here
_PutBehind: lda #%00100000
...
Setup_Vine:
        ...
        bne _NextVO               ;if set at all, don't bother to store vertical
        sta VineStart_Y_Position ;otherwise store vertical coordinate here
_NextVO: txa                      ;store object offset to next available vine slot
...
ProcLoopCommand:
	LDA	NES_BANK_VARIABLE
	JSR	UnsafeSwitchLoBank
	JSR	_ProcLoopCommand
	LDA	#01
	JMP	UnsafeSwitchLoBank

SoundEngine:
	LDA	#02
	JSR	UnsafeSwitchHiBank
	JSR	_SoundEngine
	LDA	#00
	JMP	UnsafeSwitchHiBank
